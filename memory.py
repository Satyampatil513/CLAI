...
class LongTermMemory:
    ...
    def __init__(self, db_path='memory.sqlite'):
        ...
        self.folder_embeddings = []
        self.folder_names = []
        self.file_embeddings = []
        self.file_names = []
        self.command_embeddings = []
        self.command_texts = []
        self.faiss_folder_index = None
        self.faiss_file_index = None
        self.faiss_command_index = None
    ...
    def remember_folder(self, name, path, embedding=None):
        ...
        c.execute('REPLACE INTO folders (name, path, embedding) VALUES (?, ?, ?)', (name.lower(), path, emb_blob))
        self.conn.commit()
    ...
    def remember_file(self, name, path, embedding=None):
        ...
        if embedding is not None:
            emb_blob = np.array(embedding, dtype=np.float32).tobytes()
        c.execute('REPLACE INTO files (name, path, embedding) VALUES (?, ?, ?)', (name.lower(), path, emb_blob))
        self.conn.commit()
    ...
    def log_command(self, command, output, embedding=None):
        ...
        if embedding is not None:
            emb_blob = np.array(embedding, dtype=np.float32).tobytes()
        c.execute('INSERT INTO commands (command, output, embedding) VALUES (?, ?, ?)', (command, output, emb_blob))
        self.conn.commit()
    def _load_embeddings(self, table, text_col, emb_col):
        c = self.conn.cursor()
        try:
            c.execute(f'SELECT {text_col}, {emb_col} FROM {table} WHERE {emb_col} IS NOT NULL')
...
def build_context(user_query, short_mem, long_mem, embedder):
    ...
    # 2. Search relevant folders/files/commands
